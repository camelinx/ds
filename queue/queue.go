...
func InitWithHandlers( ctx interface{ }, rlock_handler Rlock, wlock_handler Wlock, unlock_handler Unlock )( queue *Queue_t ) {
    queue = Init( )

    queue.SetLockHandlers( ctx, rlock_handler, wlock_handler, unlock_handler )

    return queue
}

func ( queue *Queue_t )SetLockHandlers( ctx interface{ }, rlock_handler Rlock, wlock_handler Wlock, unlock_handler Unlock )( ) {
    if nil == queue {
        return
    }
